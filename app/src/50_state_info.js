...
const INFO={
  ...
  tcorAgg:{t:"The aggregation rule",b:
    ...
    "<p>Flood and general property deductibles apply per location: those retained losses integrate each site's own loss ladder and sum across sites. Portfolio TCOR is therefore event-level for correlated perils and site-level for the rest - not a column total.</p>"+
    "<p>Without a results pack event table, hurricane retained falls back to a campus-comonotonic approximation over the loss ladders and is labeled as such.</p>"},
  attritional:{t:"The attritional frequency layer",b:
  ...
  lossrun:{t:"Loss-run calibration",b:
    "<p>The loss run (claims report) is the model's anchor to reality. Claimants map to sites, Coverage Major maps to peril classes, and claims group into events (named storms by name, otherwise same-class claims within three days), so a storm that produced claims at six sites is recognized as ONE occurrence - which also validates the shared-deductible aggregation.</p>"+
    "<p>Net Incurred (net of recoveries) is the calibration number. Modeled retained loss, deductible-hit frequency, and retained BI are each compared with the actuals per peril class; a ratio outside 0.5x-2x is flagged prominently. If the model disagrees with several years of actuals, the model is wrong and this tool says so.</p>"+
    "<p><b>Credibility limits:</b> a short record calibrates the BODY (attritional hits, BI) only. Named-catastrophe claims are split out; the rare-cat tail stays modeled and labeled. Open claims carry reserves that can move, so their share is flagged as development risk.</p>",
    s:"Unmatched claimants and unmapped coverage lines are listed with their dollars, never silently dropped."},
};
